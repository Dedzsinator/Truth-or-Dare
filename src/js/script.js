...
var original = {
    ...
    stageDare: [
        ["Minden játék szakaszában lesz egy <i>sexy</i> különleges Merés minden játékos számára"],
        ["Vedd le valamelyik ruhádat"],
        ["Csak a bugyidlegyen rajtad."],
        ["Minden játékosnak le kell vetkőznie"]
    ]
};
var soft = {
    name: "soft",
...
function addStageAlert(stage) {
    ...
    if (gameType.stageDare[stage] !== []) {
        swal({
            title: "<div style='color:#AD4080'><span class='glyphicon glyphicon-fire'></span> Különleges kihívás!</div>",
...
function updateView(id, type, stage) {
    ...
    if (id >= 0) {
        ...
        $("#text-action").text("Teljesítetted az " + type + " kihívást!");
    }
...
function loadJSONfromfile(file) {
    ...
    reader.onload = (function (theFile) {
        return function (e) {
            try {
                ...
                swal("Hiba", "A json fájl, feldolgozása közben : " + err, "error");
            }
        };
    }(file));
    ...
    try {
        ...
        swal("Siker!", "A fájl sikeresen betöltődött", "success");
    } catch (err) {
...
function localfileEnabling() {
    ...
    if (window.File && window.FileReader && window.FileList && window.Blob) {
        ...
        swal("Nincs szerencséd!", "A büngésződ nem támogatja az applikációt.", "error");
    }
}
...
function getJSON(source) {
    $.getJSON(source, function (json) {
        ...
        swal("Hiba", "A fájlt " + source + " nem lehet betölten, próbálkozz a beállításoknál", "error");
        localfileEnabling();
    });
}
